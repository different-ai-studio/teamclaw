...
import { KeyRound, Plus, Eye, EyeOff, Pencil, Trash2, ShieldCheck, AlertCircle, RefreshCw, Loader2 } from 'lucide-react'
...
function EnvVarDialog({ open, onOpenChange, editingEntry, onSave }: EnvVarDialogProps) {
  ...
  React.useEffect(() => {
    if (open) {
      if (editingEntry) {
        ...
        setValue('') // Don't pre-fill secret value
      } else {
        setKey('')
        setValue('')
        setDescription('')
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      await onSave(trimmedKey, value, description.trim() || undefined)
      onOpenChange(false)
    } catch (e) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing
              ? t('settings.envVars.editDescription', 'Update the value for this environment variable. Values are stored securely in your system keychain.')
              : t('settings.envVars.addDescription', 'Add a new secret that will be stored securely in your system keychain.')}
          </DialogDescription>
        </DialogHeader>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {t('settings.envVars.deleteDescription', 'Are you sure you want to delete "{{key}}"? This will remove the secret from your system keychain and cannot be undone.', { key: envVarKey })}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
...
function EnvVarRow({ entry, onEdit, onDelete }: EnvVarRowProps) {
  ...
  const handleReveal = async () => {
    ...
    try {
      ...
      // Value might not exist in keyring
      setRevealedValue(null)
    } finally {
      setLoading(false)
    }
...
export const EnvVarsSection = React.memo(function EnvVarsSection() {
  const { t } = useTranslation()
  const { envVars, isLoading, loadEnvVars, setEnvVar, deleteEnvVar, hasChanges, setHasChanges } = useEnvVarsStore()
  const workspacePath = useWorkspaceStore((s) => s.workspacePath)

  const [addDialogOpen, setAddDialogOpen] = React.useState(false)
  const [editingEntry, setEditingEntry] = React.useState<EnvVarEntry | null>(null)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {envVars.length > 0
            ? t('settings.envVars.count', '{{count}} variable(s) stored', { count: envVars.length })
            : ''}
        </p>
