...
const WATCHER_REARM_INTERVAL_MS = 10_000
const WATCHER_REARM_MAX_ATTEMPTS = 6
...
export const useKnowledgeStore = create<KnowledgeState>((set, get) => ({
  ...
  startWatcher: async () => {
    ...
    // Cancel any pending re-arm from a previous call; this run supersedes it.
    if (watcherRearmTimer) {
      clearTimeout(watcherRearmTimer)
      watcherRearmTimer = null
    }

    const arm = async (attempt: number): Promise<void> => {
      // Bail if the workspace changed out from under us mid-retry.
      if (useWorkspaceStore.getState().workspacePath !== workspacePath) return

      let allWatched = false
      try {
        // `rag_start_watcher` returns true once every configured knowledge dir
        // is watched. It watches whatever exists now and tolerates the rest, so
        // it never throws for a not-yet-created team dir.
        allWatched = await invoke<boolean>('rag_start_watcher', { workspacePath })
        console.log(`[RAG] File watcher armed (complete=${allWatched}, attempt=${attempt})`)
      } catch (error) {
        // Genuine failure (not a missing dir) — warn only, mirroring the skills
        // watcher; never surface a first-run toast.
        console.warn('[RAG] Failed to start watcher:', error)
      }

      if (allWatched || attempt >= WATCHER_REARM_MAX_ATTEMPTS) return
      // Some dirs are still missing (e.g. team clone in flight) — re-arm later.
      watcherRearmTimer = setTimeout(() => {
        watcherRearmTimer = null
        void arm(attempt + 1)
  ...
  stopWatcher: async () => {
    // Cancel any pending re-arm so a retry can't resurrect the watcher.
    if (watcherRearmTimer) {
      clearTimeout(watcherRearmTimer)
      watcherRearmTimer = null
    }
...
// Subscribe to workspace changes (desktop only — extension/web have no workspace)
if (typeof window !== 'undefined' && capabilities.workspace) {
