...
test("getMeBootstrap aggregates the caller's member actors and teams", async () => {
  ...
  const actorB = await seedActor(db, teamB.id, { userId });
  // A non-member (agent) actor for the same user must be excluded. Seed it raw
  // in a third team so it is filtered out by actor_type rather than membership.
  const teamC = await seedTeam(db, { name: "Team Gamma", slug: `gamma-${Math.random()}` });
  await db
    .insert(actors)
    .values({ teamId: teamC.id, actorType: "agent", displayName: "Agent", userId })
    .returning();

  const repo = createPgBusinessRepository({ db, userId });
  const out = await repo.getMeBootstrap();

  assert.deepEqual(Object.keys(out).sort(), ["memberActorId", "memberActorIdByTeam", "teams"].sort());
  assert.equal(out.teams.length, 2, "should surface both teams");
  ...
  assert.ok(
    out.memberActorId === actorA.id || out.memberActorId === actorB.id,
    "primary member actor id is one of the caller's team actors",
  );
});

test("getMeBootstrap returns empty shape when the user has no member actors", async () => {
  const { db } = await makeTestDb();
  const repo = createPgBusinessRepository({ db, userId: `nobody-${Math.random()}` });
  const out = await repo.getMeBootstrap();
  assert.deepEqual(out, { memberActorId: null, teams: [], memberActorIdByTeam: {} });
});
