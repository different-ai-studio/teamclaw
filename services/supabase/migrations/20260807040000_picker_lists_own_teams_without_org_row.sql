-- The team picker must list a team you are an actor in, full stop.
--
-- `member_teams` additionally required the team's org to be in `related_orgs`,
-- which is derived from `public.users.org_id`. An anonymous user has no
-- `public.users` row at all, so `related_orgs` is empty and its own team was
-- invisible — which turned into a hard failure once guests started getting
-- teams: AuthGate saw an empty list, decided this was first-team onboarding,
-- and called bootstrap again, which answered
--
--   bootstrap_current_org_team currently supports first-team onboarding only
--
-- on every launch, with no way out of the error screen.
--
-- Holding an actor in a team IS membership; the org row is bookkeeping about
-- the human, not about the membership. `public_teams` keeps the org filter —
-- discovery is deliberately scoped to orgs you belong to, and widening it
-- would expose every public team on the deployment to everyone.
