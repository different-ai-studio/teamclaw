...
const TEAM_ID: &str = "e2e-test-team";
const DESKTOP_DEVICE_ID: &str = "desktop-e2e-001";
const MOBILE_DEVICE_ID: &str = "mobile-e2e-001";
...
async fn test_desktop_online_status_retained() {
    ...
    let status_topic = format!(
        "teamclaw/{}/{}/status",
        TEAM_ID, DESKTOP_DEVICE_ID
    );

    // ── Desktop: publish retained online status ──
    let (desktop_client, mut desktop_loop) = create_client(&config, "e2e-desktop-status");
    ...
    let (mobile_client, mut mobile_loop) = create_client(&config, "e2e-mobile-status");
...
async fn test_mobile_sends_chat_request() {
    ...
    let chat_req_topic = format!(
        "teamclaw/{}/{}/chat/req",
        TEAM_ID, MOBILE_DEVICE_ID
    );

    // ── Desktop: subscribe to mobile's chat/req ──
    let (desktop_client, mut desktop_loop) = create_client(&config, "e2e-desktop-chatreq");
    let (tx, mut rx) = mpsc::channel::<MqttMessageEnvelope>(10);
    ...
    let (mobile_client, mut mobile_loop) = create_client(&config, "e2e-mobile-chatreq");
    let mobile_handle = tokio::spawn(async move {
        loop {
...
async fn test_desktop_streams_chat_response() {
    ...
    let chat_res_topic = format!(
        "teamclaw/{}/{}/chat/res",
        TEAM_ID, MOBILE_DEVICE_ID
    );

    // ── Mobile: subscribe to chat/res ──
    let (mobile_client, mut mobile_loop) = create_client(&config, "e2e-mobile-chatres");
    ...
    let (desktop_client, mut desktop_loop) = create_client(&config, "e2e-desktop-chatres");
    let desktop_handle = tokio::spawn(async move {
        loop {
            if desktop_loop.poll().await.is_err() {
                break;
...
async fn test_desktop_offline_status() {
    ...
    let (mobile_client, mut mobile_loop) = create_client(&config, "e2e-mobile-offline");
    let (tx, mut rx) = mpsc::channel::<MqttMessageEnvelope>(10);
    ...
    let (desktop_client, mut desktop_loop) = create_client(&config, "e2e-desktop-offline");
    let desktop_handle = tokio::spawn(async move {
        loop {
            if desktop_loop.poll().await.is_err() {
                break;
...
async fn test_data_sync_to_mobile() {
    ...
    let task_topic = format!("teamclaw/{}/{}/task", TEAM_ID, MOBILE_DEVICE_ID);
    let skill_topic = format!("teamclaw/{}/{}/skill", TEAM_ID, MOBILE_DEVICE_ID);
    let member_topic = format!("teamclaw/{}/{}/member", TEAM_ID, MOBILE_DEVICE_ID);

    // ── Mobile: subscribe to all sync topics ──
    let (mobile_client, mut mobile_loop) = create_client(&config, "e2e-mobile-sync");
    ...
    let (desktop_client, mut desktop_loop) = create_client(&config, "e2e-desktop-sync");
    let desktop_handle = tokio::spawn(async move {
        loop {
            if desktop_loop.poll().await.is_err() {
                break;
...
async fn test_qos1_message_delivery() {
    ...
    // ── Publisher: send message while subscriber is offline ──
    {
        ...
        tokio::time::sleep(Duration::from_millis(500)).await;
        client
            .publish(&test_topic, QoS::AtLeastOnce, false, b"offline-message")
            .await
            .unwrap();
        tokio::time::sleep(Duration::from_millis(500)).await;
        client.disconnect().await.ok();
...
async fn test_multi_device_isolation() {
    ...
    let device_a = "mobile-a-001";
    let device_b = "mobile-b-001";
    let topic_a = format!("teamclaw/{}/{}/chat/res", TEAM_ID, device_a);
    let topic_b = format!("teamclaw/{}/{}/chat/res", TEAM_ID, device_b);

    // ── Device A: subscribe to own topic ──
    let (client_a, mut loop_a) = create_client(&config, "e2e-multi-a");
    ...
    let (client_b, mut loop_b) = create_client(&config, "e2e-multi-b");
    ...
    let (desktop, mut desktop_loop) = create_client(&config, "e2e-multi-desktop");
    let desktop_handle = tokio::spawn(async move {
        loop {
            if desktop_loop.poll().await.is_err() {
                break;
    ...
    desktop
        .publish(&topic_a, QoS::AtLeastOnce, false, b"for-device-a-only")
        .await
        .unwrap();
