    ...
    fn get_socket_name(&self, path: &Option<std::path::PathBuf>) -> Result<Name<'_>, Error> {
        ...
        if cfg!(target_os = "windows") {
            ...
            socket_path
                ...
                .map_err(|e| Error::Io {
                    message: format!("Failed to create pipe name: {}", e),
                })
        } else {
            ...
            socket_path
                ...
                .map_err(|e| Error::Io {
                    message: format!("Failed to create file socket name: {}", e),
                })
        }
    }
}
...
fn handle_client<R: Runtime>(stream: UnifiedStream, app: AppHandle<R>) -> crate::Result<()> {
    ...
    let rt = tokio::runtime::Runtime::new().map_err(|e| Error::Io {
        ...
        loop {
            ...
            match reader.read_line(&mut line) {
                ...
                Err(e) => {
                    ...
                    return Err(Error::Io {
                        message: format!("Error reading from socket: {}", e),
                    });
                }
            };
                    ...
                    match writer.write_all(error_json.as_bytes()) {
                        Ok(_) => {
                            if let Err(e) = writer.flush() {
                                return Err(Error::Io {
                                    message: format!("Error flushing error response: {}", e),
                                });
                            }
                        }
                        Err(e) => {
                            return Err(Error::Io {
                                message: format!("Error writing error response: {}", e),
                            });
                        }
                    }
            ...
            match writer.write_all(response_json.as_bytes()) {
                    ...
                    match writer.flush() {
                        ...
                        Err(e) => {
                                ...
                                || e.kind() == std::io::ErrorKind::BrokenPipe
                            {
                                ...
                                return Err(Error::Io {
                                    message: format!("Error flushing response: {}", e),
                                });
                            }
                        }
                    }
                ...
                Err(e) => {
                        ...
                        || e.kind() == std::io::ErrorKind::BrokenPipe
                    {
                        ...
                        return Err(Error::Io {
                            message: format!("Error writing response: {}", e),
                        });
                    }
                }
            }
