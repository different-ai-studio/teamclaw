...
impl MqttRelay {
    ...
    async fn publish_chat_response(
        ...
        let envelope = MqttMessageEnvelope {
            ...
            timestamp: now_timestamp(),
            payload: serde_json::to_value(&ChatResponsePayload {
        ...
        self.publish_to_device(device_id, "chat/res", &envelope)
            .await
    }
    ...
    async fn handle_chat_request(
        ...
        // Build prompt parts (same format as other gateways)
    ...
    /// Complete pairing with a mobile device
    pub async fn handle_pairing_request(
        ...
        // Publish credentials to pairing topic
        if let Some(client) = self.client.lock().await.as_ref() {
            let config = self.config.read().await;
