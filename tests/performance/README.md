...
- the store bridge `window.__TEAMCLU_STORES__` **and** the `execute-js`
  listener — installed by `packages/app/src/stores/dev-expose.ts` only under
  `import.meta.env.DEV`;
- the seed surface `window.__TEAMCLU_V2_E2E__` — installed by
  `packages/app/src/lib/e2e/v2-control.ts` only when `VITE_TEAMCLU_E2E=true`.
...
VITE_TEAMCLU_E2E=true pnpm tauri:dev

# shell 2 — the suite auto-attaches to the running app via the socket
pnpm test:e2e:performance
```

`launchTeamCluApp()` reuses an already-running app when the socket is alive, so
the dev app from shell 1 is what gets measured.

### Why `VITE_TEAMCLU_E2E=true`
...
via `window.__TEAMCLU_V2_E2E__.seedConversation` (no backend, no agent
