...
pub fn fingerprint_bindings(bindings: &HashMap<String, String>) -> String {
    ...
    hkdf.expand(b"teamclaw-resolved-env-fingerprint-v1", &mut fingerprint)
...
mod tests {
    ...
    fn storage_locations_are_attached_without_values() {
        ...
        snapshot.annotate_sources(
            Some("/home/user/.teamclaw/secrets/personal-secrets.json.enc"),
            &HashMap::from([(
                "TEAM_TOKEN".to_string(),
                "/workspace/teamclaw-team/_secrets/TEAM_TOKEN.enc.json".to_string(),
            )]),
        );
