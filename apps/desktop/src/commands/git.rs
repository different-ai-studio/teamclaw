...
#[tauri::command]
pub async fn git_check_available() -> Result<GitCommandResult, String> {
    tokio::task::spawn_blocking(|| -> Result<GitCommandResult, String> {
        let output = Command::new("git")
            .no_window()
            .args(["--version"])
            .output()
...
pub async fn git_clone(
    ...
    tokio::task::spawn_blocking(move || -> Result<GitCommandResult, String> {
...
#[tauri::command]
pub async fn git_pull(path: String) -> Result<GitCommandResult, String> {
    tokio::task::spawn_blocking(move || -> Result<GitCommandResult, String> {
...
#[tauri::command]
pub async fn git_commit(path: String, message: String) -> Result<GitCommandResult, String> {
    tokio::task::spawn_blocking(move || -> Result<GitCommandResult, String> {
...
pub async fn git_status(path: String) -> Result<GitStatusResult, String> {
    tokio::task::spawn_blocking(move || -> Result<GitStatusResult, String> {
        // Get branch name
...
#[tauri::command]
pub async fn git_checkout_file(path: String, file: String) -> Result<GitCommandResult, String> {
    tokio::task::spawn_blocking(move || -> Result<GitCommandResult, String> {
...
pub async fn git_log_file(
    ...
    tokio::task::spawn_blocking(move || -> Result<Vec<GitLogEntry>, String> {
...
mod tests {
    ...
    /// Drive an async command to completion on a temporary runtime.
    /// (`git_log_file` is `pub async fn` so it can run off the Tauri main thread.)
    ...
    fn returns_commits_newest_first_with_all_fields() {
        ...
        let entries = block_on(git_log_file(
            repo_path(&repo),
            "a.txt".into(),
            Some(50),
            Some(0),
    ...
    fn initial_commit_has_empty_parent_sha() {
        ...
        let entries = block_on(git_log_file(repo_path(&repo), "a.txt".into(), None, None))
            .expect("git_log_file ok");
    ...
    fn follows_renames() {
        ...
        let entries = block_on(git_log_file(repo_path(&repo), "new.txt".into(), None, None))
            .expect("git_log_file ok");
