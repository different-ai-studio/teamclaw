...
pub async fn rag_start_watcher(
    ...
    let requested = knowledge_dirs.len();

    // If the current watcher already covers every requested dir there is
    // nothing to re-arm — avoid churning the debouncer.
    if let Some(existing) = &instance.watcher {
        if existing.watched_len() >= requested {
            return Ok(true);
        }
    }
    ...
    let all_watched = watcher.watched_len() >= requested;
    // Replacing the old watcher drops its debouncer, tearing down the stale
    // watch set before the new one takes over.
