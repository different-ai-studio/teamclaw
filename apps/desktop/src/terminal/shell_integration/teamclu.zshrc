...
if [[ -n "${TEAMCLU_USER_ZDOTDIR-}" ]]; then
  ZDOTDIR="$TEAMCLU_USER_ZDOTDIR"
else
  ZDOTDIR="$HOME"
fi
unset TEAMCLU_USER_ZDOTDIR
...
__teamclu_preexec() {
  printf '\e]633;E;%s\a' "$(__teamclu_encode "$1")"
  printf '\e]633;C\a'
}
...
__teamclu_precmd() {
  ...
  printf '\e]633;P;Cwd=%s\a' "$(__teamclu_encode "$PWD")"
  printf '\e]633;A\a'
}
...
preexec_functions=(__teamclu_preexec $preexec_functions)
precmd_functions=(__teamclu_precmd $precmd_functions)
