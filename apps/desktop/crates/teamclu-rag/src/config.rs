...
impl RagConfig {
    ...
    /// Priority: .teamclu/rag-config.json > defaults
    /// If no config file exists, creates one with default values
    pub async fn load_from_workspace(
        workspace_path: &Path,
        teamclu_dir: &str,
    ) -> anyhow::Result<Self> {
        let rag_config_path = workspace_path.join(teamclu_dir).join("rag-config.json");
        ...
        if let Err(e) = config.save_to_workspace(workspace_path, teamclu_dir).await {
            tracing::warn!(
    ...
    /// Save config to .teamclu/rag-config.json
    pub async fn save_to_workspace(
        &self,
        workspace_path: &Path,
        teamclu_dir: &str,
    ) -> anyhow::Result<()> {
        let teamclu_dir = workspace_path.join(teamclu_dir);
        tokio::fs::create_dir_all(&teamclu_dir).await?;

        let config_path = teamclu_dir.join("rag-config.json");
    ...
    pub fn db_path(&self, workspace_path: &Path, teamclu_dir: &str) -> PathBuf {
        workspace_path.join(teamclu_dir).join("knowledge.db")
    }

    /// Get BM25 index path for workspace
    pub fn bm25_index_path(&self, workspace_path: &Path, teamclu_dir: &str) -> PathBuf {
        workspace_path.join(teamclu_dir).join("bm25_index")
    }
