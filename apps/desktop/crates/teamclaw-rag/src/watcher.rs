...
impl KnowledgeWatcher {
    /// Number of knowledge directories actually being watched. Fewer than the
    /// requested set means some dirs did not exist yet (e.g. an un-cloned team
    /// `knowledge/`); the caller can re-arm later to pick them up.
...
impl KnowledgeWatcher {
    ...
    pub fn watch(
        ...
        // Watch each knowledge dir independently and tolerate per-dir failure.
        // A dir that doesn't exist yet — e.g. a team `knowledge/` behind a
        // `teamclaw-team` symlink whose target hasn't been cloned — cannot be
        // created (create_dir_all fails through a dangling symlink) or watched;
        // skip it with a warning instead of failing the whole watcher. The
        // caller re-arms once the dir materializes. Mirrors the skills watcher's
        // per-dir tolerance so first-run never surfaces a "watcher failed" error.
        let mut watched_dirs: Vec<PathBuf> = Vec::new();
        for knowledge_dir in &knowledge_dirs {
            if !knowledge_dir.exists() {
                if let Err(e) = std::fs::create_dir_all(knowledge_dir) {
                    tracing::warn!(
                        "Skipping knowledge dir (cannot create {:?}): {}",
                        knowledge_dir,
                        e
                    );
                    continue;
            ...
            match debouncer.watch(knowledge_dir, RecursiveMode::Recursive) {
                Ok(()) => {
                    tracing::info!("File watcher started for {:?}", knowledge_dir);
                    watched_dirs.push(knowledge_dir.clone());
                }
                Err(e) => {
                    tracing::warn!(
                        "Skipping knowledge dir (cannot watch {:?}): {}",
                        knowledge_dir,
                        e
                    );
                }
        ...
        let knowledge_dirs = watched_dirs;
