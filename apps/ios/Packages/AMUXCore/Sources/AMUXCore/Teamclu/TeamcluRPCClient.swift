...
///
/// `TeamcluService` used to inline this pattern at every call site —
...
public struct TeamcluRPCClient: Sendable {
    ...
    /// `Teamclu_RpcResponse`, or nil on timeout / serialization failure.
    ///
    ...
    /// requester's actor (apps/daemon/src/teamclu/rpc.rs:50-53,
    ...
    public func invoke(
        request: Teamclu_RpcRequest,
        teamID: String,
        targetActorID: String,
        timeout: TimeInterval = 10
    ) async -> Teamclu_RpcResponse? {
        ...
        return await withTaskGroup(of: Teamclu_RpcResponse?.self) { group in
            group.addTask {
                for await msg in stream {
                    ...
                    guard let response = try? Teamclu_RpcResponse(serializedBytes: msg.payload),
