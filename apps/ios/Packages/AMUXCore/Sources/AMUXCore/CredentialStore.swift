...
public struct PairingCredentials: Equatable, Sendable {
    ...
    /// True only when the user explicitly chose a custom MQTT server (Settings
    /// "Update MQTT Server" or a pairing deeplink). When false, the host was
    /// auto-applied from the bundled default — and must yield to a newer
    /// bundled default on the next launch. Without this, a host persisted by an
    /// older build (e.g. `ai.ucar.cc`) sticks forever and silently overrides a
    /// changed default across app updates.
...
public final class UserDefaultsCredentialStore: CredentialStore, @unchecked Sendable {
    ...
    public func save(_ c: PairingCredentials) throws {
        ...
        defaults.set(c.userOverride, forKey: Keys.userOverride)
    }
    ...
    public func load() throws -> PairingCredentials? {
        ...
        return PairingCredentials(
            ...
            // Absent for hosts persisted by older builds → false, so the
            // bundled default reclaims control on the next launch.
            userOverride: defaults.bool(forKey: Keys.userOverride)
        )
    }
