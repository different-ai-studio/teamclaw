...
@MainActor
final class TeamcluServiceSubscriptionTests: XCTestCase {
    ...
    func testSendMessageTwiceInSameSessionPublishesTwice() async throws {
        ...
        let firstEnv = try Teamclu_LiveEventEnvelope(serializedBytes: published[0].1)
        let secondEnv = try Teamclu_LiveEventEnvelope(serializedBytes: published[1].1)
        ...
        let firstMsg = try Teamclu_SessionMessageEnvelope(serializedBytes: firstEnv.body).message
        let secondMsg = try Teamclu_SessionMessageEnvelope(serializedBytes: secondEnv.body).message
