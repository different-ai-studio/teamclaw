...
1. On the owner's iOS device, create an agent invite — copy the `teamclu://invite?...` deeplink.
2. Run:

```bash
./target/release/amuxd init "teamclu://invite?token=<token>&broker=<mqtt-url>&username=<user>&password=<pass>"
```
