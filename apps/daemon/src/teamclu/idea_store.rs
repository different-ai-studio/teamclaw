...
impl IdeaStore {
    ...
    pub fn to_proto_idea(&self, item: &StoredIdea) -> teamclu::Idea {
        let claims = self
...
fn idea_status_to_proto(s: &str) -> teamclu::IdeaStatus {
    match s {
        "open" => teamclu::IdeaStatus::Open,
        "in_progress" => teamclu::IdeaStatus::InProgress,
        "done" => teamclu::IdeaStatus::Done,
        _ => teamclu::IdeaStatus::Unknown,
    }
}
...
mod tests {
    ...
    fn test_to_proto_idea() {
        ...
        assert_eq!(proto.status, teamclu::IdeaStatus::InProgress as i32);
