...
> 状态：**已落地**（2026-08）。依赖 P0（`TEAMCLAW_BRAND_SHORT_NAME` /
> `AMUXD_HOME`）。目标：daemon / `teamclaw-runtime-env` 与 Desktop 使用同一套
> workspace 元数据目录约定，消灭「Desktop 写 `.{brand}`、daemon 读 `.teamclaw`」。
...
| 角色 | Workspace 元数据约定 |
|---|---|
| Desktop（compile-time） | Official：`.teamclaw/teamclaw.json`；白标：`.{brand}/{brand}.json` |
| Daemon / runtime-env（今日） | **大量写死** `.teamclaw/...` |

结果：

- 干净白标 workspace（只有 `.copilot361`）→ skills / roles / 配置热更新 / allowlist / sync state **可能失效**
- 老 workspace 双目录并存 → 「看起来能跑」，但设置页改 `copilot361.json`、daemon watch `teamclaw.json` → **热更新错位**
...
2. **读**：优先 brand 规范路径；必要时兼容旧 `.teamclaw`（迁移窗口）  
3. **写**：只写 brand 规范路径，不再往幽灵 `.teamclaw` 写新数据（白标）  
4. Skills / roles / refresh watch / allowlist / active-session / opencode.runtime / sync state / team 配置读取 全部走 helper  
5. Official 路径行为与今日 **二进制兼容**（仍是 `.teamclaw`）

### 不要

- 改 `teamclaw-team` 链接名（跨 brand 固定，已拍板）  
- 改云端 schema / team 共享目录结构  
- 自动删除用户旧 `.teamclaw`（最多提示 / 可选 one-shot 复制）  
...
在 `teamclaw-runtime-env::storage_namespace`（或并列模块）提供：

```text
workspace_meta_dir_name(brand) -> ".teamclaw" | ".{brand}"
workspace_config_file_name(brand) -> "teamclaw.json" | "{brand}.json"
...
2. 否则 `brand_short_name_from_env()`（`TEAMCLAW_BRAND_SHORT_NAME`）

**已 brand-aware、保持不动：**

- `env_catalog::workspace_config_path`（已按 brand 分流）— 改为调用统一 helper，去掉重复逻辑  
- Desktop `TEAMCLAW_DIR` / `CONFIG_FILE_NAME`（compile-time）— 规则必须与 helper **字符串级一致**（加单测锁死）

**跨 brand 固定（不改）：**

| 名称 | 值 |
|---|---|
| Team link / 全局 sync 目录 | `teamclaw-team` |
...
凡今日写在 `{workspace}/.teamclaw/` 下、应由 **当前 brand meta dir** 承载的：
...
legacy    = workspace / ".teamclaw" / rel     # 仅当 brand 非 official
...
**永远写 canonical**（白标写 `.{brand}/...`，不再新建 `.teamclaw/...`）。
...
- `.{brand}/{brand}.json`  
- `.{brand}/skills`  
- （兼容期）`.teamclaw/teamclaw.json` + `.teamclaw/skills`

Official 只听 `.teamclaw/...`。
...
```text
teamclaw     → .teamclaw / teamclaw.json
teamclawdev  → .teamclaw / teamclaw.json
...
| `sync/oss/state.rs` | `TEAMCLAW_DIR` 常量 → brand resolve |
| `channels/*` | 若有 workspace 旁路 DB，一并替换 |

### PR-D：测试与文档

- 白标 fixture：仅 `.copilot361/` 无 `.teamclaw` → skills 可见、allowlist 可写、watch 触发  
- Official 回归：路径仍为 `.teamclaw`  
...
```rust
fn brand() -> String { teamclaw_runtime_env::brand_short_name_from_env() }
...
|---|---|
| Official | 无（路径不变） |
| 白标 + 双目录（今日多数） | 读仍能命中 `.teamclaw`；新写入进 `.{brand}`；长期可手动删旧目录 |
| 白标 + 仅 `.{brand}` | **修复** skills/配置（今日是坏的） |
| 白标 + 仅 `.teamclaw`（极少） | 读兼容；写开始创建 `.{brand}` |

**不做**自动 `rm -rf .teamclaw`。可选后续工具：`amuxd doctor` 报告「legacy meta present」。
...
2. Official：所有路径仍在 `.teamclaw`，现有 e2e/单测绿  
3. 兼容：workspace 仅有 `.teamclaw/skills`、无 `.copilot361/skills` → roles_skills 仍列出  
...
| 风险 | 缓解 |
|---|---|
| 漏改一处硬编码 | grep CI gate：`apps/daemon` 禁止新增字面量 `".teamclaw"`（测试 fixture 除外 allowlist） |
...
**P1 = 用与 Desktop 相同的 brand→meta-dir 规则，让 daemon 的读（含 `.teamclaw` 兼容）和写（只写 `.{brand}`）对齐；不动 team 链接名与 personal/amuxd home（P0 已覆盖）。**
...
| 层 | 关键 API / 改动 |
|---|---|
| `teamclaw-runtime-env::storage_namespace` | `workspace_meta_dir_name` / `workspace_config_*` / `resolve_*` / `write_path` / `meta_read_roots` |
...
| 防回归 | `apps/daemon/src/workspace_meta_gate.rs` — 禁止 production 硬编码 `".teamclaw"` 路径字面量 |
