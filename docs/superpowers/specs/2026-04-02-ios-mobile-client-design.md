...
### 5.0 Visual Style: iOS 26 + iMessage 风格

**设计语言：** 完全 follow iOS 26 设计规范，以 iMessage 为主要参考。轻盈对话风，亲和力强，强调"搭档"的陪伴感。
...
**元素风格：**
- 用户消息：右侧圆角气泡，带品牌色背景（iMessage 风格）
- AI 消息：无气泡，浅底色平铺全宽，视觉上像"内容区"而非"对话气泡"
- 列表项：iMessage 风格 — 头像圆形 + 标题 + 副标题 + 时间
- 导航栏：iOS 26 原生 NavigationStack 样式
- Liquid Glass 效果：底部工具栏等使用 iOS 26 的 Liquid Glass 材质

**动效：**
- 页面转场：iOS 26 原生 NavigationStack push/pop 动画
- 消息出现：轻微 fade-in + 上移，不过度花哨
- 流式打字：逐段出现，光标闪烁

**暗色模式：** V1 不做，后续可通过 SwiftUI 的 `@Environment(\.colorScheme)` 扩展。

### 5.1 Page Structure Overview (iMessage 风格)
...
- **导航栏：** 标题居中，左上角功能面板按钮，右上角团队成员按钮，标题下方显示桌面端连接状态
- **列表：** iMessage 风格 — Agent 头像、Session 标题、最后一条消息、时间
- 协作 Session 额外显示参与成员头像和最后发言人
- **底部悬浮栏：** Liquid Glass 材质，左侧搜索按钮，右侧新建 Session 按钮
...
### 5.4 Team Members Panel (右上角 👥 按钮进入)
