...
- amux tables use **RLS policies + schema-usage grant** (already granted in `20260608010000_move_teamclu_to_amux.sql`) + `PGRST_DB_SCHEMAS` (ops-set). Mirror `agents_select_if_visible` (migration `20260612010000_agents_select_self_for_self_update.sql`). Do **not** add per-table GRANT or `NOTIFY pgrst` in the migration.
...
```bash
cd /Volumes/openbeta/workspace/teamclu-v2/.worktrees/apps-module
...
Run: `npx @redocly/cli lint docs/openapi/teamclu-api.v1.yaml` (or the repo's existing OpenAPI lint command — check `package.json`/CI). If no linter is configured, run `node -e "require('yaml').parse(require('fs').readFileSync('docs/openapi/teamclu-api.v1.yaml','utf8'))"`.
...
pub fn seed_app_repo(
    ...
    run_git(&["-C", &workdir.to_string_lossy(), "config", "user.email", "daemon@teamclu"], workdir)?;
    run_git(&["-C", &workdir.to_string_lossy(), "config", "user.name", "teamclu-daemon"], workdir)?;
...
Run: `grep -rn "amuxd/templates\|templates/" apps/daemon/src` and decide the template location resolution: ship `templates/tanstack-postgres` alongside the daemon (resolved relative to the daemon binary dir, with an env override `TEAMCLU_APP_TEMPLATE_DIR`).
...
fn template_dir() -> PathBuf {
    if let Ok(p) = std::env::var("TEAMCLU_APP_TEMPLATE_DIR") {
...
Create `apps/daemon/tests/http_apps.rs` mirroring `http_sessions_runtime.rs` harness: spin the HTTP server with a token, init a bare repo as the remote, POST `/v1/apps/seed` with `TEAMCLU_APP_TEMPLATE_DIR` set to a temp template, assert 200 and that the bare repo received a commit. (Reuse the `TestApp`/`cfg_identity` helpers' style.)
...
Add `apps: AppsBackend;` to the `TeamCluBackend` interface (next to `sessions`).
